...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (held)
    {
        PushStyleColor(ImGuiCol_Text, ImGuiCol_TextSelectedBg);
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
    else if (hovered)
    {
        PushStyleColor(ImGuiCol_Text, ImVec4(0.2f, 0.2f, 0.2f, 0.8f));
        RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
        PopStyleColor();
    }
    else
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    const ImVec2 padding = ImVec2(style.FramePadding.x , style.FramePadding.x);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
...
bool ImGui::ImageAnimButton(ImTextureID user_texture_id, ImTextureID user_texture_id2, ImTextureID user_texture_id3, const ImVec2& size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (pressed)
    {
        window->DrawList->AddImage(user_texture_id3, image_bb.Min, image_bb.Max);
    }
    else if (hovered)
    {
        window->DrawList->AddImage(user_texture_id2, image_bb.Min, image_bb.Max);
    }
    else
    {
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec4 vecMainColor = style.Colors[ImGuiCol_CheckMark];
    const ImVec4 vecSecondaryColor = vecMainColor - ImVec4(0.15f, 0.f, 0.f, 0.f) + ImVec4(0.f, 0.f, 0.05f, 0.f);
    ...
    const ImRect check_bblabel(pos, pos + ImVec2(square_sz, square_sz));
    const ImRect check_bb(pos + ImVec2(0.f, 4.f), pos + ImVec2(square_sz, square_sz) + ImVec2(0.f, 4.f));
    ...
    else if (*v)
    {
		...
		// gradient
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 6.0f)), ImMax(1.0f, IM_FLOOR(square_sz / 6.0f)));
        
        window->DrawList->AddRectFilledMultiColor(check_bb.Min + pad, check_bb.Max - pad, GetColorU32(vecSecondaryColor), GetColorU32(vecSecondaryColor), GetColorU32(vecMainColor), GetColorU32(vecMainColor));
    }

    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bblabel.Max.x + style.ItemInnerSpacing.x + 6.f, check_bblabel.Min.y + style.FramePadding.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    // modified by qo0
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f, label_size.x > 0.0f ? label_size.y : 0.0f), window->DC.CursorPos + ImVec2(w, (label_size.x > 0.0f ? label_size.y : 0.0f) + h - 3.5f));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max - ImVec2((label_size.x > 0.0f ? style.ItemInnerSpacing.x + h : 0.0f), 0.0f) + ImVec2(0.f, 6.f));
    ...
    const ImVec4 vecMainColor = g.ActiveId == id ? style.Colors[ImGuiCol_SliderGrabActive] : style.Colors[ImGuiCol_SliderGrab];
    const ImVec4 vecSecondaryColor = vecMainColor - ImVec4(0.15f, 0.f, 0.f, 0.f) + ImVec4(0.f, 0.f, 0.05f, 0.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        // slider grab
        if (g.ActiveId == id)
            window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(vecMainColor), style.FrameRounding);
        else
            window->DrawList->AddRectFilledMultiColor(grab_bb.Min, grab_bb.Max, GetColorU32(vecMainColor), GetColorU32(vecMainColor), GetColorU32(vecSecondaryColor), GetColorU32(vecSecondaryColor));
    }
    ...
    ImVec2 vecValueSize = CalcTextSize(value_buf, NULL, true);

    RenderText(ImVec2(grab_bb.Max.x - CalcTextSize(value_buf).x - 1.0f + (vecValueSize.x / 2.f), grab_bb.Min.y + 2.f + style.FramePadding.y), value_buf, value_buf_end);
